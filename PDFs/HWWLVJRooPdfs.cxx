...
Double_t Simp_Gaus(Double_t x, Double_t mu, Double_t sigma){
  Double_t norm = 1/(sigma * TMath::Sqrt(2*TMath::Pi()));
  return norm * TMath::Exp((x-mu)*(x-mu)/(2*sigma*sigma));
}
/////end angle functions
...
ClassImp(Simple_Gaus)

Simple_Gaus::Simple_Gaus(const char *name, const char *title,
			RooAbsReal& _x,
			RooAbsReal& _mu,
			RooAbsReal& _sigma) :
RooAbsPdf(name,title),
...
Simple_Gaus::Simple_Gaus(const Simple_Gaus& other, const char* name) :
  RooAbsPdf(other,name),
  x("x","x",this,other.x),
  mu("mu","mu",this,other.mu),
  sigma("sigma","sigma",this,other.sigma){}

Double_t Simple_Gaus::evaluate() const {
  return Simp_Gaus(x,mu,sigma);
}

...
#color command: \033[0;42;33m     \033[0m
...
class doFit_wj_and_wlvj:
    ...
    def __init__(self, in_channel, in_mj_min=40, in_mj_max=150, in_mlvj_min=900., in_mlvj_max=4500., fit_model="ExpN", fit_model_alter="ExpTail"):
        ...
        #angular results
        self.fitresultstheta1 = []
        self.fitresultstheta2 = []
        self.fitresultsthetastar = []
        self.fitresultsphi = []
        self.fitresultsphi1 = []
        self.fitresultsphi2 = []
        ...
        rrv_mass_j = RooRealVar("rrv_mass_j","M_{Softdrop_PUPPI}",(in_mj_min+in_mj_max)/2.,in_mj_min,in_mj_max,"GeV");
        ...
        rrv_theta1 = RooRealVar("rrv_theta1", "theta1", 0.0, -1.0, +1.0);
        rrv_theta2 = RooRealVar("rrv_theta2", "theta2", 0.5, 0.0, 1.0);
        rrv_thetastar = RooRealVar("rrv_thetastar", "thetastar", 0.0, -1.0, +1.0);
        rrv_phi1 = RooRealVar("rrv_phi1", "phi1", 0.0, -PI, +PI);
        rrv_phi2 = RooRealVar("rrv_phi2", "phi2", 0.0, -PI, +PI);
        rrv_phi = RooRealVar("rrv_phi", "phi", 1.6, 0.0, PI);
        ...
        getattr(self.workspace4fit_,"import")(rrv_theta1);
        getattr(self.workspace4fit_,"import")(rrv_theta2);
        getattr(self.workspace4fit_,"import")(rrv_thetastar);
        getattr(self.workspace4fit_,"import")(rrv_phi1);
        getattr(self.workspace4fit_,"import")(rrv_phi2);
        getattr(self.workspace4fit_,"import")(rrv_phi);
        ...
        self.plotsDir = 'angle_plots_%s_%s_%s_%s' %(self.channel,self.wtagger_label,options.mlvj_lo,options.mlvj_hi)
        if not os.path.isdir("angle_cards_%s_%s_%s_%s"%(self.channel,self.wtagger_label,options.mlvj_lo,options.mlvj_hi)):
                os.system("mkdir angle_cards_%s_%s_%s_%s"%(self.channel,self.wtagger_label,options.mlvj_lo,options.mlvj_hi));
        self.rlt_DIR="angle_cards_%s_%s_%s_%s/"%(self.channel,self.wtagger_label,options.mlvj_lo,options.mlvj_hi)
    ...
    def method_legend4Plot(self, plot, left=1, isFill=1, x_offset_low=0., y_offset_low=0., x_offset_high =0., y_offset_high =0., TwoCoulum =1., isalpha=False, ismj=False):
...
objName ==objName_before ):
    ...
    def method_get_canvas(self,cname,isalpha=False):
    ...
    def method_draw_canvas(self, in_obj,in_directory, in_file_name, is_range=0, logy=0, frompull=0, isalpha=0, fix_axis=0, force_plots=0):
      
        if options.noplots and not force_plots:
          return 0

        print "############### draw the canvas without pull ########################"
        cMassFit = self.method_get_canvas(in_obj.GetName())#TCanvas("cMassFit","cMassFit", 600,600);
    ...
    def method_draw_canvas_with_pull(self, rrv_x, datahist, mplot, mplot_pull,ndof,parameters_list,in_directory, in_file_name, in_model_name="", show_constant_parameter=0, logy=0,ismj=0, fix_axis=0, force_plots=0):# mplot + pull
        ...
        cMassFit = self.method_get_canvas(mplot.GetName())
        ...
        #self.method_draw_canvas(mplot,in_directory,string_file_name.Data(),0,logy,1,0,fix_axis);

    #################################################################################################
    #################################################################################################

    def method_calculate_chi2(self,hist,rrv_x,mplot_orig,ndof,ismj):
    ...
    def method_get_pull(self, rrv_x, mplot_orig):
    ...
    def method_getData_PoissonInterval(self,data_obs,mplot):
    ...
    def method_get_mlvj_Model(self,label, mlvj_region):
    ...
    def method_get_mj_Model(self,label):
    ...
    def method_get_General_mlvj_Model(self, label, mlvj_region="_sig"):
        ...
        model_General = self.method_get_mlvj_Model(label,mlvj_region);
    ...
    def method_get_General_mj_Model(self, label ):
        print "########### Fixing a general mj model  ############"
        rdataset_General_mj = self.workspace4fit_.data("rdataset%s_%s_mj"%(label,self.channel))
        print "\033[0;42;33mPrinting out rdataset%s_%s_mj"%(label,self.channel), " for your consideration.\033[0m"
        model_General = self.method_get_mj_Model(label);
    ...
    def method_get_WJets_mj_Model(self,label):
        ...
        print "\033[0;42;33mPrinting out rdataset%s_%s_mj"%(label,self.channel), " for your consideration.\033[0m"
        model_WJets = self.method_get_mj_Model(label);
    ...
    def method_fix_Model(self, label, mlvj_region="total_region",mass_spectrum="_mlvj"):
        ...
        model = self.method_get_mlvj_Model(label,mlvj_region);
        model.Print();
        rdataset.Print();
    ...
    def method_fix_Pdf(self,model_pdf,argset_notparameter):
    ...
    def method_make_Pdf(self, label, in_model_name, mass_spectrum="_mj", ConstraintsList=[],ismc = 0): #METHOD 15
        ...
        if TString(mass_spectrum).Contains("theta1"): rrv_x = self.workspace4fit_.var("rrv_theta1");
        ...
        if in_model_name == "ExpN":
            ...
            print "\033[0;44;33mWE ARE PRINTING model_pdf\033[0m"
            model_pdf.Print();
            print "\033[0;44;33mWE HAVE PRINTED model_pdf\033[0m"
        ...
        #angular variable possibilities
        if in_model_name == "Simple_Gaus":
            print "################### SIMPLE GAUSSIAN #########################"
            rrv_mu_Gaus    = RooRealVar("rrv_mu_Gaus"   +label+"_"+self.channel,"rrv_mu_Gaus"   +label+"_"+self.channel,0.0,-0.5,+0.5);
            rrv_sigma_Gaus = RooRealVar("rrv_sigma_Gaus"+label+"_"+self.channel,"rrv_sigma_Gaus"+label+"_"+self.channel,0.5,0.01,100.0);

            model_pdf = ROOT.Simple_Gaus("model_pdf"+label+"_"+self.channel,"model_pdf"+label+"_"+self.channel, rrv_x, rrv_mu_Gaus, rrv_sigma_Gaus)
            print "\033[0;44;33mWE ARE PRINTING model_pdf\033[0m"
            model_pdf.Print();
            print "\033[0;44;33mWE HAVE PRINTED model_pdf\033[0m"

        #if in_model_name == "Alt_Exp":
            #print "########### Exp = levelled exp funtion for W+jets mlvj ############"
            #rrv_c_alt_Exp = RooRealVar("rrv_c_alt_Exp"+label+"_"+self.channel,"rrv_c_alt_Exp"+label+"_"+self.channel,-0.05,-0.1,0.);
            #model_pdf = ROOT.RooExponential("model_pdf"+label+"_"+self.channel+mass_spectrum,"model_pdf"+label+"_"+self.channel+mass_spectrum,rrv_x,rrv_c_alt_Exp);
    ...
    def method_make_Model(self, label, in_model_name, mass_spectrum="_mj", ConstraintsList=[], ismc_wjet=0, area_init_value=500): #METHOD 16
      ...
      model_pdf = self.method_make_Pdf(label,in_model_name,mass_spectrum,ConstraintsList,ismc_wjet) #METHOD 15
      print "######## Model Pdf ########"        
      model_pdf.Print();
      rrv_number.Print();
    ...
    def method_fit_mlvj_in_Mj_sideband(self, label, mlvj_region, mlvj_model,logy=0):
        ...
        model_WW_backgrounds    = self.method_get_General_mlvj_Model("_WW","_sb").clone("WW")
        number_WW_sb_mlvj            = self.workspace4fit_.var("rrv_number_WW_sb_%s_mlvj"%(self.channel))
        model_WZ_backgrounds    = self.method_get_General_mlvj_Model("_WZ","_sb").clone("WZ")
        number_WZ_sb_mlvj            = self.workspace4fit_.var("rrv_number_WZ_sb_%s_mlvj"%(self.channel))
        model_TTbar_backgrounds = self.method_get_General_mlvj_Model("_TTbar","_sb").clone("TTbar")
        number_TTbar_sb_mlvj         = self.workspace4fit_.var("rrv_number_TTbar_sb_%s_mlvj"%(self.channel))
        model_STop_backgrounds  = self.method_get_General_mlvj_Model("_STop","_sb").clone("STop")
        number_STop_sb_mlvj          = self.workspace4fit_.var("rrv_number_STop_sb_%s_mlvj"%(self.channel))

        ### Make the Pdf for the WJets
        model_pdf_WJets = self.method_make_Pdf("%s_sb_from_fitting"%(label), mlvj_model,"_mlvj");
        ...
        if TString(label).Contains("_WJets0"):
            ...
            self.method_getData_PoissonInterval(rdataset_data_mlvj,mplot);

            mplot.GetYaxis().SetRangeUser(1e-2,mplot.GetMaximum()*1.2);  

            ### Add the legend to the plot 
            #self.leg=self.method_legend4Plot(mplot,0,1,0., 0.16, 0.16, 0.1);
            self.leg=self.method_legend4Plot(mplot,0,1,0.25,0.,0.1,0.,0);
            ...
            mplot_pull = self.method_get_pull(rrv_mass_lvj,mplot);
            ...
            #@#self.method_draw_canvas_with_pull( rrv_mass_lvj,datahist,mplot, mplot_pull,ndof,parameters_list,"%s/m_lvj_fitting/"%(self.plotsDir), "m_lvj_sb%s"%(label),"",1,1)
            self.method_draw_canvas_with_pull( rrv_mass_lvj,rdataset_data_mlvj,mplot, mplot_pull,ndof,parameters_list,"%s/m_lvj_fitting/"%(self.plotsDir), "m_lvj_sb%s"%(label),"",1,1,0,1)
        ...
        self.method_get_WJets_mlvj_correction_sb_to_sig(label,mlvj_model);

        ### Fix the pdf of signal, TTbar, STop WW and WZ in the signal region 
        self.method_fix_Model("_TTbar","_sig","_mlvj")
        self.method_fix_Model("_STop","_sig","_mlvj")
        self.method_fix_Model("_WW","_sig","_mlvj")
        self.method_fix_Model("_WZ","_sig","_mlvj")

    #################################################################################################
    #################################################################################################

    ##### Function that calculates the normalization inside the mlvj signal region
    def method_get_mlvj_normalization_insignalregion(self, label, model_name=""):
        ...
        self.file_out.write( "\n%s++++++++++++++++++method_get_mlvj_normalization_insignalregion++++++++++++++++++"%(label) )
    ...
    def method_get_WJets_mlvj_correction_sb_to_sig(self,label, mlvj_model):
        ...
        ### Add the legend
        self.leg= self.method_legend4Plot(mplot,1,0, 0, 0., 0., -0.1, 0., True);
        mplot.addObject(self.leg);
        ...
        self.method_draw_canvas(mplot,"%s/other/"%(self.plotsDir),"correction_pdf%s_%s_M_lvj_sig_to_sideband"%(label,mlvj_model),0,1,0,1);
        ...
        if 'WJets0' in label:
            ...
            leg        = self.method_legend4Plot(mplot,1,0, 0, 0.1, 0., 0.075, 0., True)
    ...
    def method_fit_WJetsNorm(self, scaleJetMass = 0): # to get the normalization of WJets in sig
        #->not used anymore, just make the prefit plot
        self.method_mj_prefit_plot()

    #################################################################################################
    #################################################################################################

    def method_mj_prefit_plot(self): 
        ...
        model_TTbar = self.method_get_General_mj_Model("_TTbar");
        model_STop  = self.method_get_General_mj_Model("_STop");
        model_WW    = self.method_get_General_mj_Model("_WW");
        model_WZ    = self.method_get_General_mj_Model("_WZ");
        model_WJets = self.method_get_General_mj_Model('_WJets0');
        ...
        ### legend of the plot
        self.leg = self.method_legend4Plot(mplot,0,1,0.3,0.,0.,0.,0,0,1);
        ...
        mplot_pull = self.method_get_pull(rrv_mass_j,mplot);

        self.method_draw_canvas_with_pull( rrv_mass_j,rdataset_data_mj,mplot,mplot_pull,0,parameters_list,"%s/m_j_fitting/"%(self.plotsDir), "m_j_prefit",'',1,0,1)

    #################################################################################################
    #################################################################################################

    def method_theta1_fit(self,in_file_name, label, in_range, model_name, ismc=0): #my theta1 fitting method METHOD 21.5
        print "######################fitting theta1s###############################"

        rrv_theta1 = self.workspace4fit_.var("rrv_theta1")
        #rrv_theta2 = self.workspace4fit_.var("rrv_theta2")
        #rrv_thetastar = self.workspace4fit_.var("rrv_thetastar")
        #rrv_phi1 = self.workspace4fit_.var("rrv_phi1")
        #rrv_phi2 = self.workspace4fit_.var("rrv_phi2")
        #rrv_phi = self.workspace4fit_.var("rrv_phi")

        # RooDataSet("rdataset4fit"+label+"_"+self.channel+"_angles_sb",   "rdataset4fit"+label+"_"+self.channel+"_angles_sb",
        rdataset = self.workspace4fit_.data("rdataset4fit"+label+in_range+"_"+self.channel+"_angles") #DO NOT CHANGE THIS AGAIN!
        #print "HERE WE ARE ABOUT TO ACCESS A \033[0;41;33mNULL-POINTER\033[0m:rdataset4fit"+label+in_range+"_"+self.channel+"_angles"
        rdataset.Print()

        model = self.method_make_Model(label+in_range,model_name,"theta1",[],ismc) #METHOD 16
        ...
        rfresult.Print();  
        self.fitresultstheta1.append(rfresult)
        rfresult.SetName("rfresult"+label+in_range+"_"+self.channel+"_theta1")
        getattr(self.workspace4fit_,'import')(rfresult)

        ## plot the result
        mplot = rrv_theta1.frame(RooFit.Title("theta1_"+in_range+" fitted by "+model_name), RooFit.Bins(rrv_theta1.getBins()));
        ...
        nBinX = mplot.GetNbinsX();
        ndof  = nBinX-nPar;
        print "Here we are entering a \033[0;42;33mC++ exception\033[0m: ", rdataset.GetName()+"_binnedClone"
        datahist = rdataset.binnedClone( rdataset.GetName()+"_binnedClone",rdataset.GetName()+"_binnedClone" )
        rdataset.Print()

        ## get the pull 
        
        mplot_pull      = self.method_get_pull(rrv_theta1,mplot);
        parameters_list = model.getParameters(rdataset);
        ###mplot.GetYaxis().SetRangeUser(1e-2,mplot.GetMaximum()*1.2);#fix for theta1s
 
        self.method_draw_canvas_with_pull( rrv_theta1, datahist,mplot, mplot_pull,ndof,parameters_list,"%s/m_lvj_fitting/"%(self.plotsDir), in_file_name,"theta1"+in_range+model_name, show_constant_parameter, logy);
        #after this, there is divergence of how to handle things
    ...
    def method_fit_mlvj_model_single_MC(self,in_file_name, label, in_range, mlvj_model, deco=0, show_constant_parameter=0, logy=0, ismc=0): #first main fit function, sideband and signal #METHOD 22.
        ...
        model = self.method_make_Model(label+in_range,mlvj_model,"_mlvj",constrainslist,ismc); #METHOD 16.
        ...
        print "Here we are NOT entering a \033[0;42;33mC++ exception\033[0m: ",  rdataset.GetName()+"_binnedClone"
        datahist = rdataset.binnedClone( rdataset.GetName()+"_binnedClone",rdataset.GetName()+"_binnedClone" )
        rdataset.Print()
        ...
        self.method_draw_canvas_with_pull( rrv_mass_lvj, datahist,mplot, mplot_pull,ndof,parameters_list,"%s/m_lvj_fitting/"%(self.plotsDir), in_file_name,"m_lvj"+in_range+mlvj_model, show_constant_parameter, logy);
        
        #@# make missing plot
        if 'TTbar' in in_file_name and 'sb' in in_range:
                self.method_draw_canvas_with_pull( rrv_mass_lvj, datahist,mplot, mplot_pull,ndof,parameters_list,"%s/m_lvj_fitting/"%(self.plotsDir), in_file_name,"m_lvj"+in_range+mlvj_model, show_constant_parameter, 1)
        ...
        if deco :
            ...
            self.leg = self.method_legend4Plot(mplot_deco,0); ## add the legend                
            mplot_deco.addObject(self.leg);

            self.method_draw_canvas( mplot_deco, "%s/other/"%(self.plotsDir), "m_lvj"+label+in_range+in_range+mlvj_model+"_deco",0,logy)
    ...
    def method_fit_mj_single_MC(self,in_file_name, label, in_model_name, additioninformation=""): #second main fit function #METHOD 23.
        ...
        model = self.method_make_Model(label,in_model_name); #METHOD 16.
	...
	#	print dataHist.GetY()[i]
        ...
        #print self.method_calculate_chi2(rdataset_mj,rrv_mass_j, mplot, ndof,1)
        mplot.Print()

        print "Here we are NOT entering a \033[0;42;33mC++ exception\033[0m: ", rdataset_mj.GetName()+"_binnedClone"
        ...
        self.method_draw_canvas_with_pull( rrv_mass_j,datahist,mplot, mplot_pull,ndof,parameters_list,"%s/m_j_fitting/"%(self.plotsDir), label+in_file_name, in_model_name)
    ...
    def method_get_mj_and_mlvj_dataset(self,in_file_name, label):# to get the shape of m_lvj,jet_mass="jet_mass_pr" #METHOD 24

        if not options.read_trees:
                #self.method_get_mj_and_mlvj_dataset_from_file(in_file_name, label)
                print "You need to use the option read trees."; exit(33);
        else:
                print "################### method_get_mj_and_mlvj_dataset : ",in_file_name,"  ",label,"  ##################";
                ...
                rrv_mass_lvj = self.workspace4fit_.var("rrv_mass_lvj")
                #mark place here
                rrv_theta1 = self.workspace4fit_.var("rrv_theta1")# "theta1")# 0.0, -1.0, +1.0);
                rrv_theta2 = self.workspace4fit_.var("rrv_theta2")# "theta2")# 0.5, 0.0, 1.0);
                rrv_thetastar = self.workspace4fit_.var("rrv_thetastar")# "thetastar")# 0.0, -1.0, +1.0);
                rrv_phi1 = self.workspace4fit_.var("rrv_phi1")# "phi1")# 0.0, -PI, +PI);
                rrv_phi2 = self.workspace4fit_.var("rrv_phi2")# "phi2")# 0.0, -PI, +PI);
                rrv_phi = self.workspace4fit_.var("rrv_phi")# "phi", 1.6, 0.0, PI);
                ...
                # NEW DATASETS FOR ANGLES
                rdataset_sig_angles   = RooDataSet("rdataset"+label+"_sig"+"_"+self.channel+"_angles",   "rdataset"+label+"_sig"+"_"+self.channel+"_angles",   RooArgSet(rrv_theta1,rrv_theta2,rrv_thetastar,rrv_phi1,rrv_phi2,rrv_phi,rrv_mass_j,rrv_mass_lvj,rrv_weight),RooFit.WeightVar(rrv_weight) );
                
                rdataset_sb_angles    = RooDataSet("rdataset"+label+"_sb"+"_"+self.channel+"_angles",    "rdataset"+label+"_sb"+"_"+self.channel+"_angles",    RooArgSet(rrv_theta1,rrv_theta2,rrv_thetastar,rrv_phi1,rrv_phi2,rrv_phi,rrv_mass_j,rrv_mass_lvj,rrv_weight),RooFit.WeightVar(rrv_weight) );
                
                rdataset_sb_lo_angles = RooDataSet("rdataset"+label+"_sb_lo"+"_"+self.channel+"_angles", "rdataset"+label+"_sb_lo"+"_"+self.channel+"_angles", RooArgSet(rrv_theta1,rrv_theta2,rrv_thetastar,rrv_phi1,rrv_phi2,rrv_phi,rrv_mass_j,rrv_mass_lvj,rrv_weight),RooFit.WeightVar(rrv_weight) );
                
                rdataset_sb_hi_angles = RooDataSet("rdataset"+label+"_sb_hi"+"_"+self.channel+"_angles", "rdataset"+label+"_sb_hi"+"_"+self.channel+"_angles", RooArgSet(rrv_theta1,rrv_theta2,rrv_thetastar,rrv_phi1,rrv_phi2,rrv_phi,rrv_mass_j,rrv_mass_lvj,rrv_weight),RooFit.WeightVar(rrv_weight) );

                #DUPLICATE DATASETS FOR ANGLES
                rdataset4fit_sig_angles   = RooDataSet("rdataset4fit"+label+"_sig"+"_"+self.channel+"_angles",   "rdataset4fit"+label+"_sig"+"_"+self.channel+"_angles",   RooArgSet(rrv_theta1,rrv_theta2,rrv_thetastar,rrv_phi1,rrv_phi2,rrv_phi,rrv_mass_j,rrv_mass_lvj,rrv_weight),RooFit.WeightVar(rrv_weight) );
                
                rdataset4fit_sb_angles    = RooDataSet("rdataset4fit"+label+"_sb"+"_"+self.channel+"_angles",    "rdataset4fit"+label+"_sb"+"_"+self.channel+"_angles",    RooArgSet(rrv_theta1,rrv_theta2,rrv_thetastar,rrv_phi1,rrv_phi2,rrv_phi,rrv_mass_j,rrv_mass_lvj,rrv_weight),RooFit.WeightVar(rrv_weight) );
                #print "HERE WE ARE SETTING A \033[0;42;33mC++ EXCEPTION\033[0m? rdataset4fit"+label+"_sb"+"_"+self.channel+"_angles"
                
                rdataset4fit_sb_lo_angles = RooDataSet("rdataset4fit"+label+"_sb_lo"+"_"+self.channel+"_angles", "rdataset4fit"+label+"_sb_lo"+"_"+self.channel+"_angles", RooArgSet(rrv_theta1,rrv_theta2,rrv_thetastar,rrv_phi1,rrv_phi2,rrv_phi,rrv_mass_j,rrv_mass_lvj,rrv_weight),RooFit.WeightVar(rrv_weight) );
                
                rdataset4fit_sb_hi_angles = RooDataSet("rdataset4fit"+label+"_sb_hi"+"_"+self.channel+"_angles", "rdataset4fit"+label+"_sb_hi"+"_"+self.channel+"_angles", RooArgSet(rrv_theta1,rrv_theta2,rrv_thetastar,rrv_phi1,rrv_phi2,rrv_phi,rrv_mass_j,rrv_mass_lvj,rrv_weight),RooFit.WeightVar(rrv_weight) );
                ...
                for i in range(treeIn.GetEntries()):
                    ...
                    if self.isGoodEvent == 1:
                        ...
                        rrv_theta1.setVal(treeIn.costheta1)
                        rrv_theta2.setVal(fabs(treeIn.costheta2))
                        rrv_thetastar.setVal(treeIn.costhetastar)
                        rrv_phi1.setVal(treeIn.phi1)
                        rrv_phi2.setVal(treeIn.phi2)
                        rrv_phi.setVal(fabs(treeIn.phi))
                        
                        rrv_mass_lvj.setVal(treeIn.MWW_SD);
                        #sideband lo only
                        if (tmp_jet_mass >= self.mj_sideband_lo_min and tmp_jet_mass < self.mj_sideband_lo_max):
                            rdataset_sb_lo_mlvj.add( RooArgSet( rrv_mass_lvj, rrv_theta1, rrv_theta2, rrv_thetastar, rrv_phi1, rrv_phi2, rrv_phi ), tmp_event_weight );
                            rdataset4fit_sb_lo_mlvj.add( RooArgSet( rrv_mass_lvj, rrv_theta1, rrv_theta2, rrv_thetastar, rrv_phi1, rrv_phi2, rrv_phi ), tmp_event_weight4fit );
                            
                            rdataset_sb_mlvj.add( RooArgSet( rrv_mass_lvj, rrv_theta1, rrv_theta2, rrv_thetastar, rrv_phi1, rrv_phi2, rrv_phi ), tmp_event_weight );
                            rdataset4fit_sb_mlvj.add( RooArgSet( rrv_mass_lvj, rrv_theta1, rrv_theta2, rrv_thetastar, rrv_phi1, rrv_phi2, rrv_phi ), tmp_event_weight4fit );

                            data_category.setLabel("sideband");
                            combData.add(RooArgSet(rrv_mass_lvj, rrv_theta1, rrv_theta2, rrv_thetastar, rrv_phi1, rrv_phi2, rrv_phi,data_category),tmp_event_weight);
                            combData4fit.add(RooArgSet(rrv_mass_lvj, rrv_theta1, rrv_theta2, rrv_thetastar, rrv_phi1, rrv_phi2, rrv_phi,data_category),tmp_event_weight4fit);
                                    
                        #signal region, blind data if needed
                        if tmp_jet_mass >= self.mj_signal_min and tmp_jet_mass < self.mj_signal_max:
                            rdataset_sig_mlvj.add( RooArgSet( rrv_mass_lvj, rrv_theta1, rrv_theta2, rrv_thetastar, rrv_phi1, rrv_phi2, rrv_phi ), tmp_event_weight );
                            rdataset4fit_sig_mlvj.add( RooArgSet( rrv_mass_lvj, rrv_theta1, rrv_theta2, rrv_thetastar, rrv_phi1, rrv_phi2, rrv_phi ), tmp_event_weight4fit );
                        ...
                        if tmp_jet_mass >= self.mj_sideband_hi_min and tmp_jet_mass < self.mj_sideband_hi_max:
                            rdataset_sb_hi_mlvj.add( RooArgSet( rrv_mass_lvj, rrv_theta1, rrv_theta2, rrv_thetastar, rrv_phi1, rrv_phi2, rrv_phi ), tmp_event_weight );
                            rdataset4fit_sb_hi_mlvj.add( RooArgSet( rrv_mass_lvj, rrv_theta1, rrv_theta2, rrv_thetastar, rrv_phi1, rrv_phi2, rrv_phi ), tmp_event_weight4fit );
                        ...
                        if True:#'data' in label:
                          rdataset_mj.add(RooArgSet(rrv_mass_j, rrv_theta1, rrv_theta2, rrv_thetastar, rrv_phi1, rrv_phi2, rrv_phi), tmp_event_weight )
                          rdataset4fit_mj.add(RooArgSet(rrv_mass_j, rrv_theta1, rrv_theta2, rrv_thetastar, rrv_phi1, rrv_phi2, rrv_phi), tmp_event_weight4fit )
                          #datasets for simultaneaous fit in combine
                          ##sideband lo only
                          if tmp_jet_mass>= self.mj_sideband_lo_min and tmp_jet_mass < self.mj_sideband_lo_max:
                            rrv_mass_j_sb_lo.setVal(tmp_jet_mass)
                            dataset_mj_sb_lo.add(RooArgSet(rrv_mass_j_sb_lo,rrv_mass_lvj,rrv_theta1, rrv_theta2, rrv_thetastar, rrv_phi1, rrv_phi2, rrv_phi), 1)
                          ##sideband hi
                          if tmp_jet_mass >= self.mj_sideband_hi_min and tmp_jet_mass < self.mj_sideband_hi_max:
                            rrv_mass_j_sb_hi.setVal(tmp_jet_mass)
                            dataset_mj_sb_hi.add(RooArgSet(rrv_mass_j_sb_hi,rrv_mass_lvj,rrv_theta1, rrv_theta2, rrv_thetastar, rrv_phi1, rrv_phi2, rrv_phi), 1)        
                          ##signal
                          if tmp_jet_mass >= self.mj_signal_min and tmp_jet_mass < self.mj_signal_max:
                            rrv_mass_j_sig.setVal(tmp_jet_mass)
                            dataset_mj_sig.add(RooArgSet(rrv_mass_j_sig,rrv_mass_lvj,rrv_theta1, rrv_theta2, rrv_thetastar, rrv_phi1, rrv_phi2, rrv_phi), 1)
                ...
                getattr(self.workspace4fit_,"import")(rdataset_sb_angles);
                getattr(self.workspace4fit_,"import")(rdataset_sb_lo_angles);
                getattr(self.workspace4fit_,"import")(rdataset_sig_angles);
                getattr(self.workspace4fit_,"import")(rdataset_sb_hi_angles);
                getattr(self.workspace4fit_,"import")(rdataset4fit_sb_angles);
                getattr(self.workspace4fit_,"import")(rdataset4fit_sb_lo_angles);
                getattr(self.workspace4fit_,"import")(rdataset4fit_sig_angles);
                getattr(self.workspace4fit_,"import")(rdataset4fit_sb_hi_angles);
                ...
                if 'WJets0_' in label:
                        fileOut        = TFile.Open('angle_cards_%s_%s_900_4500/datasets_%s_%s.root'%(self.channel,self.wtagger_label,self.channel,self.wtagger_label),'recreate')
                else:
                        fileOut        = TFile.Open('angle_cards_%s_%s_900_4500/datasets_%s_%s.root'%(self.channel,self.wtagger_label,self.channel,self.wtagger_label),'update')
 ...
 #   def method_get_mj_and_mlvj_dataset_from_file(self,in_file_name, label):
 ...
 #       fileIn.Close()
    ...
    def method_get_data(self):
        print "############### method_get_data ########################"
        self.method_get_mj_and_mlvj_dataset(self.file_data,"_data")
    ...
    def method_fit_STop(self): #METHOD 27
        print "############################## method_fit_STop  #################################"
        self.method_get_mj_and_mlvj_dataset(self.file_STop_mc,"_STop")
        self.method_fit_mj_single_MC(self.file_STop_mc,"_STop","ExpGaus"); #METHOD 23.
        self.method_fit_mlvj_model_single_MC(self.file_STop_mc,"_STop","_sb", "Exp", 0, 0, 1, 0); #METHOD 22.
        self.method_fit_mlvj_model_single_MC(self.file_STop_mc,"_STop","_sig","Exp", 0, 0, 1, 0); #METHOD 22.
        #self.method_fit_mlvj_model_single_MC(self.file_STop_mc,"_STop","_sb", "Alt_Exp", 0, 0, 1, 0);
        #self.method_fit_mlvj_model_single_MC(self.file_STop_mc,"_STop","_sig","Alt_Exp", 0, 0, 1, 0);
        
        self.method_theta1_fit(self.file_STop_mc, "_STop", "_sb",  "Simple_Gaus", 0) #METHOD 21.5 21.5
        self.method_theta1_fit(self.file_STop_mc, "_STop", "_sig", "Simple_Gaus", 0) #METHOD 21.5 21.5
    ...
    def method_fit_WW(self):
        print "############################# method_fit_WW ################################"
        ### Build the dataset
        self.method_get_mj_and_mlvj_dataset(self.file_WW_mc,"_WW")
        self.method_fit_mj_single_MC(self.file_WW_mc,"_WW","2GausWW"); #METHOD 23.      
        self.method_fit_mlvj_model_single_MC(self.file_WW_mc,"_WW","_sb","Exp", 0, 0, 1, 0); #METHOD 22.
        self.method_fit_mlvj_model_single_MC(self.file_WW_mc,"_WW","_sig",self.MODEL_4_mlvj, 0, 0, 1, 0); #METHOD 22.

        #self.method_theta1_fit(self.file_WW_mc, "_WW", "_sb",  "Exp", 0) #METHOD 21.5 21.5
        #self.method_theta1_fit(self.file_WW_mc, "_WW", "_sig", "Exp", 0) #METHOD 21.5 21.5    
        print "________________________________________________________________________"
    ...
    def method_fit_WZ(self):
        print "############################# method_fit_WZ ################################"
        ### Build the dataset
        self.method_get_mj_and_mlvj_dataset(self.file_WZ_mc,"_WZ")
        self.method_fit_mj_single_MC(self.file_WZ_mc,"_WZ","2GausWZ"); #METHOD 23.      
        self.method_fit_mlvj_model_single_MC(self.file_WZ_mc,"_WZ","_sb","ExpN", 0, 0, 1, 0); #METHOD 22.
        self.method_fit_mlvj_model_single_MC(self.file_WZ_mc,"_WZ","_sig","ExpN", 0, 0, 1, 0); #METHOD 22.

        #self.method_theta1_fit(self.file_WZ_mc, "_WZ", "_sb",  "Exp", 0) #METHOD 21.5 21.5
        #self.method_theta1_fit(self.file_WZ_mc, "_WZ", "_sig", "Exp", 0) #METHOD 21.5 21.5
        print "________________________________________________________________________"   
    ...
    def method_fit_TTbar(self):
        print "################################ method_fit_TTbar #########################################"
        ### Build the dataset
        self.method_get_mj_and_mlvj_dataset(self.file_TTbar_mc,"_TTbar")# to get the shape of m_lvj
        self.method_fit_mj_single_MC(self.file_TTbar_mc,"_TTbar","2Gaus_ErfExp"); #METHOD 23.
        self.method_fit_mlvj_model_single_MC(self.file_TTbar_mc,"_TTbar","_sb","ExpTail",1, 0, 0, 0); #METHOD 22.
        self.method_fit_mlvj_model_single_MC(self.file_TTbar_mc,"_TTbar","_sig","ExpTail",1, 0, 1, 0); #METHOD 22.

        #self.method_theta1_fit(self.file_TTbar_mc, '_TTbar', "_sb",  "Exp", 0) #METHOD 21.5 21.5
        #self.method_theta1_fit(self.file_TTbar_mc, '_TTbar', "_sig", "Exp", 0) #METHOD 21.5 21.5
        print "________________________________________________________________________"
    ...
    def method_fit_WJets(self):
        print "######################### method_fit_WJets ########################"
        ### Build the dataset
        self.method_get_mj_and_mlvj_dataset(self.file_WJets0_mc,"_WJets0")# to get the shape of m_lvj
        #self.method_get_mj_and_mlvj_dataset(self.file_WJets0_mc,"_WJets1")# to get the shape of m_lvj
        ### Fit in mj depends on the mlvj lower limit -> fitting the turn on at low mass or not
        self.method_fit_mj_single_MC(self.file_WJets0_mc,"_WJets0","ChiSqBern") #METHOD 23.
        #self.method_fit_mj_single_MC(self.file_WJets0_mc,"_WJets1","User1");
        #### Fit the mlvj in sb_lo, signal region using two different model as done in the mj

        self.method_fit_mlvj_model_single_MC(self.file_WJets0_mc,"_WJets0","_sb",self.MODEL_4_mlvj, 0, 0, 1, 1); #METHOD 22.
        self.method_fit_mlvj_model_single_MC(self.file_WJets0_mc,"_WJets0","_sig",self.MODEL_4_mlvj, 0, 0, 1, 1); #METHOD 22.
        #self.method_fit_mlvj_model_single_MC(self.file_WJets0_mc,"_WJets1","_sb",self.MODEL_4_mlvj_alter, 0, 0, 1, 1);
        #self.method_fit_mlvj_model_single_MC(self.file_WJets0_mc,"_WJets1","_sig",self.MODEL_4_mlvj_alter, 0, 0, 1, 1);

        #self.method_theta1_fit(self.file_WJets0_mc, '_WJets0', "_sb",  "Exp", 0) #METHOD 21.5 21.5
        #self.method_theta1_fit(self.file_WJets0_mc, '_WJets0', "_sig", "Exp", 0) #METHOD 21.5 21.5
        print "________________________________________________________________________"

    #################################################################################################
    #################################################################################################

    def method_fit_type(self,bcknd):
        print "################# method fit for ", bcknd, " #########################"

        func_form_mj = ""; func_form_mlvj_sb = ""; func_form_mlvj_sig = "";
        file_name = "";
        
        if (bcknd == "STop"):
            func_form_mj = "ExpGaus";
            func_form_mlvj_sb = "Exp";
            file_name = self.file_STop_mc;

        if (bcknd == "WW"):
            func_form_mj = "2GausWW";
            func_form_mlvj_sb = "Exp"; func_form_mlvj_sig = self.MODEL_4_mlvj;
            file_name = self.file_WW_mc;
            
        if (bcknd == "WZ"):
            func_form_mj = "2GausWZ";
            func_form_mlvj_sb = "ExpN";
            file_name = self.file_WZ_mc;
        
        if (bcknd == "TTbar"):
            func_form_mj = "2Gaus_ErfExp";
            func_form_mlvj_sb = "ExpTail";
            file_name = self.file_TTbar_mc;
        
        if (bcknd == "WJets"):
            func_form_mj = "ChiSqBern";
            func_form_mlvj_sb = self.MODEL_4_mlvj;
            file_name = self.file_WJets0_mc;

        if (func_form_mlvj_sig == ""): func_form_mlvj_sig = func_form_mlvj_sb;
        
        self.method_get_mj_and_mlvj_dataset(file_name, "_"+bcknd);
        self.method_fit_mj_single_MC(file_name, "_"+bcknd, func_form_mj);
        ...
        self.method_fit_mlvj_model_single_MC(file_name, "_"+bcknd,"_sb",  func_form_mlvj_sb,  a, 0, c, d);
        self.method_fit_mlvj_model_single_MC(file_name, "_"+bcknd,"_sig", func_form_mlvj_sig, a, 0, 1, d);
        print "________________________________________________________________________"

        

    ##### Analysis with sideband alpha correction 
    def method_analysis_sideband_correction_method1(self):

        print "##################### Start sideband correction full analysis ##############"

        ### Fit all MC components in both mj and mlvj
        print "################### method_fit_AllSamples_Mj_and_Mlvj #####################"
        self.method_fit_WJets()
        self.method_fit_TTbar()
        self.method_fit_WW()
        self.method_fit_WZ()
        self.method_fit_STop()
        #self.method_fit_type("WJets")
        #self.method_fit_type("TTbar")
        #self.method_fit_type("WW")
        #self.method_fit_type("WZ")
        #self.method_fit_type("STop")
        print "________________________________________________________________________"

        ### take the real data
        self.method_get_data()
        ### fit the WJets Normalization into the signal region -> no jet mass fluctuation has been done
        self.method_fit_WJetsNorm();
        ### fit data in the mlvj low sideband with two different models
        #self.method_fit_mlvj_in_Mj_sideband("_WJets1","_sb",self.MODEL_4_mlvj_alter,1)
        self.method_fit_mlvj_in_Mj_sideband("_WJets0","_sb",self.MODEL_4_mlvj,1)

        ### Prepare the workspace and datacards     
        self.method_prepare_limit("sideband_correction_method1",1,0,0)
        ### finale plot and check of the workspace
        self.method_read_workspace(1)
    ...
    def method_prepare_limit(self,mode, isTTbarFloating=0, isVVFloating=0, isSTopFloating=0):
        ...
        print "####################### method_prepare_limit for %s method ####################"%(mode);
        ...
        for label in ['TTbar','STop','WW','WZ']:
            self.method_fix_Pdf(self.workspace4limit_.pdf('%s_mlvj_sig_%s'%(label,self.channel)), RooArgSet(rrv_x) ); 
        ...
        ### Save the workspace
        self.method_save_workspace_to_file();

    #################################################################################################
    #################################################################################################

    #### Method used in order to save the workspace in a output root file
    def method_save_workspace_to_file(self):
    ...
    def method_read_workspace(self, logy=0):
        ...
        self.method_getData_PoissonInterval(data_obs,mplot);
        model_Total_background_MC.plotOn(mplot,RooFit.Normalization(scale_number_Total_background_MC),RooFit.Invisible());
        mplot_pull = self.method_get_pull(rrv_x,mplot);

        ### Plot the list of floating parameters and the uncertainty band is draw taking into account this floating list defined in the method_prepare_limit
        ...
        mplot.Print();
        leg =        self.method_legend4Plot(mplot,0,1,0.25,0.,0.1,0.,0);
        ...
        self.method_draw_canvas_with_pull( rrv_x,datahist,mplot, mplot_pull,ndof,parameters_list,"%s/m_lvj_fitting/"%(self.plotsDir),"check_workspace_for_limit","",0,1,0,1);
    
    #################################################################################################
    #################################################################################################

### funtion to run the complete alpha analysis
def method_pre_limit_sb_correction(method, channel, in_mj_min=40, in_mj_max=150, in_mlvj_min=900, in_mlvj_max=4500, fit_model="ExpN", fit_model_alter="ExpTail"): 

    print "#################### method_pre_limit_sb_correction: channel %s, max and min mJ %f-%f, max and min mlvj %f-%f, fit model %s and alternate %s ######################"%(channel,in_mj_min,in_mj_max,in_mlvj_min,in_mlvj_max,fit_model,fit_model_alter);
    
    boostedW_fitter=doFit_wj_and_wlvj(channel,in_mj_min,in_mj_max,in_mlvj_min,in_mlvj_max,fit_model,fit_model_alter);
    getattr(boostedW_fitter,"method_analysis_sideband_correction_%s"%(method) )();
...
if __name__ == '__main__':
    ...
    method_pre_limit_sb_correction("method1",channel,40,150,options.mlvj_lo,4500,"ExpN","ExpTail")
